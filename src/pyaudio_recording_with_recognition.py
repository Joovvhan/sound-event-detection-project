...
answer_list = ['alarms', 'bird', 'bug', 'doors', 'fan', 'jazz', 'keyboard', 'rain', 'speaking']

class basic_rnn_model(nn.Module):
    def __init__(self):
        super(basic_rnn_model, self).__init__()
        self.gru_1 = nn.GRU(160, 128, 1, bidirectional=True)
...
def update_plot(frame):
    ...
    S = librosa.feature.melspectrogram(y=plotdata/2**15, sr=RATE, n_mels=160)
...
#         lines3[0].set_ydata(np.mean(S, axis=0) > 0.2)
    ...
    if voice_activated and VAD[-1] < 1:
        ...
        if idx > 15 and idx < 50:
            ...
            recording_array = plotdata[-44100-2000:-2000].astype(np.int16)
            wavfile.write(file_name, 44100, recording_array)
            play(song)
            ...
            mel_band = librosa.filters.mel(44100, int(40 / 1000 * 44100), n_mels=160)
            f, t, Zxx = scipy_signal.stft(recording_array, fs=44100, nperseg=int(40 / 1000 * 44100), noverlap=0)
            Sxx = np.abs(Zxx)
            Sxx = np.matmul(mel_band, Sxx)
...
#   model_path = glob.glob('notebooks/models/*')[selection]
    model_path = 'model/simple_test.pt'
